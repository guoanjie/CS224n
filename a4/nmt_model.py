...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(self.model_embeddings.embed_size, self.hidden_size, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        init_decoder_hidden = self.h_projection(last_hidden.permute(1, 0, 2).reshape(len(source_lengths), -1))
        init_decoder_cell   = self.c_projection(  last_cell.permute(1, 0, 2).reshape(len(source_lengths), -1))
